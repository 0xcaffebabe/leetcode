 ...
 * [1462] 课程表 IV
 * 
 * 69/69 cases passed (26 ms)
 * Your runtime beats 32.95 % of java submissions
 * Your memory usage beats 62.02 % of java submissions (47.2 MB)
 */
...
class Solution {
    ...
    public List<Boolean> checkIfPrerequisite(int numCourses, int[][] prerequisites, int[][] queries) {
        n = numCourses;
        reacheable = new boolean[numCourses][numCourses];
        g = new boolean[numCourses][numCourses];
        for(var p: prerequisites) g[p[0]][p[1]] = true;
        for(int i = 0; i < numCourses; i++) 
            markReacheable(i, i, new boolean[n]);
        
        List<Boolean> ans = new ArrayList<>();
        for(int i = 0; i < queries.length; i++)
            ans.add(reacheable[queries[i][0]][queries[i][1]]);
        return ans;
    }

    void markReacheable(int cur, int v, boolean[] visted) {
        if (visted[v]) return;
        visted[v] = true;
        reacheable[cur][v] = true;
        for(int i = 0; i < n; i++) {
            if (g[v][i] && v != i)
                markReacheable(cur, i, visted);
        }
    }
}
// @lc code=end
